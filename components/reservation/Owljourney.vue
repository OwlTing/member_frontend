    ...
    <ul class="reservationBox list-unstyled">
        <li v-for="(itm1, key1) in reservation" :key="key1">
                                ...
                                <BRow>
                                    <BCol md="2" sm="2" xs="12" class="d-sm-none d-md-block">
                                        <NuxtLink :to="itm3.url" target="_blank">
                                            <BImg v-if="itm3.image_url" :src="itm3.image_url" />
                                            <BImg v-else src="https://stage.owljourney.com/img/default-room.b8e70292.jpg" />
                                        </NuxtLink>
                                    </BCol>
                                    ...
                                    <BCol md="3" sm="4" xs="12">
                                        <div class="text-end">
                                            ...
                                            <div class="mt-1 text-danger" style="font-size:14px" v-if="itm3.discount > 0">{{ $t('reservation.discount') }}: -{{ itm3.discount }}</div>
            ...
            <BContainer fluid class="mt-3 py-2" style="border-top:1px solid #ccc">
                <BRow>
                    <BCol>
                        <div><span style="font-size:12px">{{ $t('m.status') }}:</span> {{ itm1.status }}</div>
...
<style scoped>
    ...
    #layout-reservation .reservationBox img {border-radius:6px}
