    ...
    <ul class="reservationBox list-unstyled">
        <li v-for="(itm1, key1) in reservation" :key="key1">
            ...
            <BContainer fluid class="mt-3 py-2" style="border-top:1px solid #ccc">
                <BRow>
                    <BCol>
                        <div><span style="font-size:12px">{{ $t('reservation.payment') }}:</span> {{ itm1.payment_name }} <span style="font-size:12px">({{ itm1.status }})</span></div>
                    ...
                    <BCol class="text-end">
                        <div>
                            ...
                            <span class="price" style="font-size:20px; font-weight:bold">{{ itm1.currency }} {{ itm1.total }}</span>
                        </div>
